      ...
      <div className="flex gap-2 flex-wrap">
        {["", "pending", "approved", "rejected"].map((s) => (
          <button key={s} onClick={() => setFilterStatus(s)}
            className={`px-3 py-1.5 rounded-xl text-xs font-medium transition-all capitalize ${filterStatus === s ? "bg-primary-600 text-white" : "btn-secondary"}`}>
            {s || "All"}
          </button>
        ))}
      </div>
                ...
                <div className="space-y-3 border-t border-primary-100 dark:border-primary-800 pt-4">
                    ...
                    {["approve","reject"].map((a) => (
                      <button key={a} onClick={() => setReviewForm((f) => ({ ...f, action: a }))}
                        className={`flex-1 py-2 rounded-xl text-xs font-medium capitalize transition-all
                          ${reviewForm.action === a ? (a === "approve" ? "bg-primary-600 text-white" : "bg-red-500 text-white") : "btn-secondary"}`}>
                        {a}
                      </button>
                    ))}
                  </div>
                  ...
                  <button onClick={() => handleReview(selected.id)} disabled={reviewing}
                    className={`w-full flex items-center justify-center gap-2 py-2 rounded-xl text-sm font-medium transition-all
                      ${reviewForm.action === "approve" ? "bg-primary-600 hover:bg-primary-700 text-white" : "bg-red-500 hover:bg-red-600 text-white"}`}>
